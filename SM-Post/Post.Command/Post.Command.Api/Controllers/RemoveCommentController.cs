...
namespace Post.Command.Api.Controllers
{
    ...
    public class RemoveCommentController : ControllerBase
    {
        ...
        public RemoveCommentController(ILogger<RemoveCommentController> logger, 
        ...
        public async Task<ActionResult> RemoveCommentAsync(Guid id, RemoveCommentCommand command) {
            try {
                ...
                _logger.Log(LogLevel.Warning, ex, "Could not retrieve aggregate, client pass a incorrect id");
                return BadRequest(new BaseResponse {
                    Message = ex.Message
                });
            }
            catch (Exception ex) {
                const string SAFE_ERROR_MESSAGE = "Error while processing request removing comment!";
